...
def analyze_pendulum(file_path, start_dt, end_dt,mode,output_path = ""):
    ...
    # === GRAPHIQUE ===
    plt = plot_resume(df_resume, start_dt, end_dt)

    if mode == "save":
        # === SAVE GRAPHIQUE ===
        save_plot(plt,output_path)
        # === SAVE SUMMARY TXT===
        write_summary_pendulum(df, start_dt, end_dt,output_path)
        # === EXPORT EXCEL À 2 ONGLET ===
        export_excel(df, df_resume,output_path)
    else:
        plt.show()


#analyze_pendulum(r"C:\Users\nathans\Desktop\git_musee\Energy\pendulum\LOG\LOG_DATA\2025\2025-07-01_to_2025-07-27\LOG.TXT" , "2025-07-01 00:00:00", "2025-07-27 23:59:59","save",DOWNLOAD_DIR)